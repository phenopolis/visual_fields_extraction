...
#   image object (as a cv2 image i.e. NumPy array), uses OpenCV image detection
...
class Hvf_Object:
    ...
    KEYLABEL_AGE = "age"
    KEYLABEL_GENDER = "gender"
    KEYLABEL_FIXATION_MONITOR = "fixation_monitor"
    KEYLABEL_FIXATION_TARGET = "fixation_target"
    KEYLABEL_STIMULUS = "stimulus"
    KEYLABEL_VISUAL_ACUITY = "visual_acuity"
    KEYLABEL_BACKGROUND = "background"
    ###
    KEYLABEL_ID = "id"
    ...
    # Fixation Monitor
    HVF_GAZE = "Gaze/Blind Spot"
    HVF_TRACK = "Gaze Tracking"  # guessing
    ...
    # Stimulus
    HVF_3_WHITE = "III, White"
    # TODO: what else?

    # Background
    HVF_31_5_ASB = "31.5 ASB"
    # TODO: what else?
    ...
    def get_hvf_object_from_image(cls, hvf_image, debug_dir=""):
        ...
        hvf_obj = Hvf_Object(
            ...
            abs_dev_value_array,
            pat_dev_value_array,
            abs_dev_percentile_array,
            pat_dev_percentile_array,
            hvf_image,
        )
    ...
    def get_hvf_object_from_text(cls, hvf_text):
        ...
        raw_val_plot = Hvf_Plot_Array.get_plot_from_array(
            Hvf_Plot_Array.PLOT_RAW, Hvf_Plot_Array.PLOT_VALUE, raw_val_plot_array
        )
        ...
        # Make a plot wrapper for the array:
        total_dev_val_plot = Hvf_Plot_Array.get_plot_from_array(
            Hvf_Plot_Array.PLOT_TOTAL_DEV, Hvf_Plot_Array.PLOT_VALUE, abs_val_plot
        )
        ...
        # Make a plot wrapper for the array:
        pat_dev_val_plot = Hvf_Plot_Array.get_plot_from_array(
            Hvf_Plot_Array.PLOT_PATTERN_DEV, Hvf_Plot_Array.PLOT_VALUE, pat_val_plot
        )
        ...
        # Make a plot wrapper for the array:
        total_dev_perc_plot = Hvf_Plot_Array.get_plot_from_array(
            Hvf_Plot_Array.PLOT_TOTAL_DEV, Hvf_Plot_Array.PLOT_PERC, abs_perc_plot
        )
        ...
        pat_dev_perc_plot = Hvf_Plot_Array.get_plot_from_array(
            Hvf_Plot_Array.PLOT_PATTERN_DEV, Hvf_Plot_Array.PLOT_PERC, pat_perc_plot
        )
        ...
        # Instantiate a new object:
        hvf_obj = Hvf_Object(
            hvf_dict, raw_val_plot, total_dev_val_plot, pat_dev_val_plot, total_dev_perc_plot, pat_dev_perc_plot, None
        )
    ...
    def get_hvf_object_from_dicom(cls, dicom_ds):
        ...
        if hvf_metadata[Hvf_Object.KEYLABEL_LATERALITY] == Hvf_Object.HVF_OD:
            ...
            sphere = str(
                dicom_ds.OphthalmicPatientClinicalInformationRightEyeSequence[0]
            ...
            sphere = str(
                dicom_ds.OphthalmicPatientClinicalInformationLeftEyeSequence[0]
                .RefractiveParametersUsedOnPatientSequence[0]
                .SphericalLensPower
            )
            cylinder = str(
        ...
        is_pattern_generated = (
            dicom_ds.VisualFieldTestPointSequence[0]
            .VisualFieldTestPointNormalsSequence[0]
        ...
        for datapoint in dicom_ds.VisualFieldTestPointSequence:
            ...
            is_pattern_generated = datapoint.VisualFieldTestPointNormalsSequence[
                0
            ].GeneralizedDefectCorrectedSensitivityDeviationFlag

            if is_pattern_generated == "YES":
                pdv_val = int(
                    datapoint.VisualFieldTestPointNormalsSequence[0].GeneralizedDefectCorrectedSensitivityDeviationValue
                )
                pdv_array[r, c] = Hvf_Value.get_value_from_display_string(str(pdv_val))

                pdp_perc = datapoint.VisualFieldTestPointNormalsSequence[
                    0
        ...
        if (
            hvf_metadata.get(Hvf_Object.KEYLABEL_FIELD_SIZE) == Hvf_Object.HVF_24_2
            or hvf_metadata.get(Hvf_Object.KEYLABEL_FIELD_SIZE) == Hvf_Object.HVF_30_2
        ):
            blind_spot_r = 4
            blind_spot_c = 0
            ...
            tdv_array[blind_spot_c, blind_spot_r + 1] = Hvf_Value.get_value_from_display_string(
                Hvf_Value.VALUE_NO_VALUE
            )

            tdp_array[blind_spot_c, blind_spot_r] = Hvf_Perc_Icon.get_perc_icon_from_char(
                Hvf_Perc_Icon.PERC_NO_VALUE_CHAR
            )
            tdp_array[blind_spot_c, blind_spot_r + 1] = Hvf_Perc_Icon.get_perc_icon_from_char(
                Hvf_Perc_Icon.PERC_NO_VALUE_CHAR
            )

            if is_pattern_generated == "YES":
                pdv_array[blind_spot_c, blind_spot_r] = Hvf_Value.get_value_from_display_string(
                    Hvf_Value.VALUE_NO_VALUE
                )
                pdv_array[blind_spot_c, blind_spot_r + 1] = Hvf_Value.get_value_from_display_string(
                    Hvf_Value.VALUE_NO_VALUE
                )

                pdp_array[blind_spot_c, blind_spot_r] = Hvf_Perc_Icon.get_perc_icon_from_char(
                    Hvf_Perc_Icon.PERC_NO_VALUE_CHAR
                )
                pdp_array[blind_spot_c, blind_spot_r + 1] = Hvf_Perc_Icon.get_perc_icon_from_char(
                    Hvf_Perc_Icon.PERC_NO_VALUE_CHAR
                )

        raw_array_plot = Hvf_Plot_Array.get_plot_from_array(
            Hvf_Plot_Array.PLOT_RAW, Hvf_Plot_Array.PLOT_VALUE, raw_array
        )
        tdv_array_plot = Hvf_Plot_Array.get_plot_from_array(
            Hvf_Plot_Array.PLOT_TOTAL_DEV, Hvf_Plot_Array.PLOT_VALUE, tdv_array
        )
        tdp_array_plot = Hvf_Plot_Array.get_plot_from_array(
            Hvf_Plot_Array.PLOT_TOTAL_DEV, Hvf_Plot_Array.PLOT_PERC, tdp_array
        )
        pdv_array_plot = Hvf_Plot_Array.get_plot_from_array(
            Hvf_Plot_Array.PLOT_PATTERN_DEV, Hvf_Plot_Array.PLOT_VALUE, pdv_array
        )
        pdp_array_plot = Hvf_Plot_Array.get_plot_from_array(
            Hvf_Plot_Array.PLOT_PATTERN_DEV, Hvf_Plot_Array.PLOT_PERC, pdp_array
        )

        hvf_obj = Hvf_Object(
            hvf_metadata, raw_array_plot, tdv_array_plot, pdv_array_plot, tdp_array_plot, pdp_array_plot, None
        )
    ...
    def get_best_match(cls, alist: list, field: str) -> str:
        ...
        for item in alist:
            score = fuzz.partial_ratio(item, field)
            if score > best_score:
                best_match = item
                best_score = score
    ...
    def equals(self, arg_obj):
        ...
        if not (
            self.raw_value_array.get_display_string_list(Hvf_Object.SERIALIZATION_DELIMITER_CHAR)
            == arg_obj.raw_value_array.get_display_string_list(Hvf_Object.SERIALIZATION_DELIMITER_CHAR)
        ):
            return False

        if not (
            self.abs_dev_value_array.get_display_string_list(Hvf_Object.SERIALIZATION_DELIMITER_CHAR)
            == arg_obj.abs_dev_value_array.get_display_string_list(Hvf_Object.SERIALIZATION_DELIMITER_CHAR)
        ):
            return False

        if not (
            self.pat_dev_value_array.get_display_string_list(Hvf_Object.SERIALIZATION_DELIMITER_CHAR)
            == arg_obj.pat_dev_value_array.get_display_string_list(Hvf_Object.SERIALIZATION_DELIMITER_CHAR)
        ):
            return False

        if not (
            self.abs_dev_percentile_array.get_display_string_list(Hvf_Object.SERIALIZATION_DELIMITER_CHAR)
            == arg_obj.abs_dev_percentile_array.get_display_string_list(Hvf_Object.SERIALIZATION_DELIMITER_CHAR)
        ):
            return False

        if not (
            self.pat_dev_percentile_array.get_display_string_list(Hvf_Object.SERIALIZATION_DELIMITER_CHAR)
            == arg_obj.pat_dev_percentile_array.get_display_string_list(Hvf_Object.SERIALIZATION_DELIMITER_CHAR)
        ):
            return False
    ...
    def get_header_metadata_from_hvf_image(hvf_image_gray, layout_version):
        ...
        hvf_image_gray = cv2.bitwise_not(
            cv2.adaptiveThreshold(hvf_image_gray, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY_INV, 11, 5)
        )
        ...
        # For simplicity, just combine all into same text string for analysis
        if (
            (layout_version == Hvf_Object.HVF_LAYOUT_V1)
        ...
        # 1. OCR is not 100% accurate, so need to use "fuzzy" matching (i.e., the closest fit)
        ...
        #    make them most accurate (e.g., we know laterality is either Right or Left, so
        ...
        #    fewer options) and increases search speed (fewer things to search through)
        # 4. As we identify things (if it's a sufficiently high match), we can delete them
        ...
        field, tokenized_header_middle_list = Regex_Utils.fuzzy_regex("Background:", tokenized_header_middle_list)

        alist = [Hvf_Object.HVF_31_5_ASB]

        field = Hvf_Object.get_best_match(alist, field)

        hvf_metadata[Hvf_Object.KEYLABEL_BACKGROUND] = field

        # ===== STIMULUS DETECTION =====
        field, tokenized_header_middle_list = Regex_Utils.fuzzy_regex("Stimulus:", tokenized_header_middle_list)
        alist = [Hvf_Object.HVF_3_WHITE]

        field = Hvf_Object.get_best_match(alist, field)

        hvf_metadata[Hvf_Object.KEYLABEL_STIMULUS] = field

        # ===== FIXATION_TARGET DETECTION =====
        field, tokenized_header1_list = Regex_Utils.fuzzy_regex("Fixation Target:", tokenized_header1_list)
        alist = [Hvf_Object.HVF_CENTRAL]

        field = Hvf_Object.get_best_match(alist, field)

        hvf_metadata[Hvf_Object.KEYLABEL_FIXATION_TARGET] = field

        # ===== FIXATION_MONITOR DETECTION =====
        field, tokenized_header1_list = Regex_Utils.fuzzy_regex("Fixation Monitor:", tokenized_header1_list)
        alist = [Hvf_Object.HVF_GAZE, Hvf_Object.HVF_TRACK]

        field = Hvf_Object.get_best_match(alist, field)

        hvf_metadata[Hvf_Object.KEYLABEL_FIXATION_MONITOR] = field

        # ===== GENDER DETECTION =====
        field, tokenized_header1_list = Regex_Utils.fuzzy_regex("Gender: ", tokenized_header1_list)
        alist = [Hvf_Object.HVF_MALE, Hvf_Object.HVF_FEMALE, Regex_Utils.REGEX_FAILURE]
        field = Hvf_Object.get_best_match(alist, field)
        hvf_metadata[Hvf_Object.KEYLABEL_GENDER] = field

        # ===== AGE DETECTION =====
        field, tokenized_header4_list = Regex_Utils.fuzzy_regex("Age: ", tokenized_header4_list)
        hvf_metadata[Hvf_Object.KEYLABEL_AGE] = field
        ...
        if layout_version == Hvf_Object.HVF_LAYOUT_V3:
            field, tokenized_header1_list = Regex_Utils.fuzzy_regex_middle_field(
                "| OD |", "(.*)", tokenized_header1_list
            )
    ...
    def get_abs_raw_val_plot(hvf_image_gray, layout_version):
        ...
        # icon plot
        return Hvf_Plot_Array.get_plot_from_image(
            hvf_image_gray, plot_type, icon_type, y_ratio, y_size, x_ratio, x_size
        )
